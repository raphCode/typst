...
use super::{cast, dict, func, ty, Array, Dict, Func, IntoValue, Value};
use crate::diag::{bail, At, SourceResult, StrResult};
use crate::eval::{Args, Vm};
...
/// pattern can be either a string or a [regular expression]($type/regex). This
/// makes the methods quite versatile.
///
/// All lengths and indices are expressed in terms of UTF-8 characters. Indices
/// are zero-based and negative indices wrap around to the end of the string.
///
/// ## Example { #example }
/// ```example
/// #"hello world!" \
...
#[ty("str")]
#[constructor(Str::convert_func())]
#[scope(
    scope.define("len", Str::len_func());
    scope.define("first", Str::first_func());
    scope.define("last", Str::last_func());
    scope.define("at", Str::at_func());
    scope.define("slice", Str::slice_func());
    scope.define("clusters", Str::clusters_func());
    scope.define("codepoints", Str::codepoints_func());
    scope.define("from-unicode", Str::from_unicode_func());
    scope.define("to-unicode", Str::to_unicode_func());
    scope.define("contains", Str::contains_func());
    scope.define("starts-with", Str::starts_with_func());
    scope.define("ends-with", Str::ends_with_func());
    scope.define("find", Str::find_func());
    scope.define("position", Str::position_func());
    scope.define("match", Str::match_func());
    scope.define("matches", Str::matches_func());
    scope.define("replace", Str::replace_func());
    scope.define("trim", Str::trim_func());
    scope.define("split", Str::split_func());
    scope
)]
...
impl Str {
    ...
    /// [`str.to-unicode`]($type/str.to-unicode) and
    /// [`str.from-unicode`]($type/str.from-unicode).
    ///
    /// ## Example { #example }
    /// ```example
    /// #str(10) \
    ...
    /// Extract the first grapheme cluster after the specified index. Returns
    ...
    /// Category: foundations
    #[func(Str)]
    pub fn to_unicode(self) -> StrResult<u32> {
        Ok(Value::Str(self).cast::<char>()? as u32)
    }
...
///
/// Can be used as a [show rule selector]($styling/#show-rules) and with [string
/// methods]($type/string) like `find`, `split`, and `replace`.
///
...
/// Category: foundations
#[ty("regex")]
#[constructor(Regex::construct_func())]
#[derive(Clone)]
